...
**현재 상태:** 일부 화면은 로컬 오버레이/모달이 이미 존재하지만, 정식 저장/검증/API 연동은 미완료
**구현 방법:** 현재 `OverlayPanel` 또는 shadcn `Dialog` 중 화면 패턴에 맞는 방식을 유지하면서 API 연동
...
> 참고: 실제 구현 시 `src/components/frontend/*`에 이미 들어간 로컬 오버레이를 먼저 확인하고, 불필요한 중복 모달을 새로 만들지 않는다.
