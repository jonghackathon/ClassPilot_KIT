# Step 6: AI 연동 (코파일럿, 에세이 피드백, 성장보고서, Whisper, 이탈예측 배치)
...
| # | 파일 | 작업 |
|---|------|------|
| 1 | `src/app/api/copilot/sessions/route.ts` | POST — 세션 생성 |
| 2 | `src/app/api/copilot/questions/route.ts` | POST (SSE 또는 질문 처리) — 질문 → 4카드 생성 |
| 3 | `src/app/api/copilot/sessions/[id]/route.ts` | PATCH — 세션/상태 기록 |
...
| # | 파일 | 작업 |
|---|------|------|
| 1 | `src/app/api/reports/generate/route.ts` | POST — 성장 리포트 생성 |
...
| # | 파일 | 작업 |
|---|------|------|
| 1 | `src/app/api/recordings/route.ts` | POST — 파일 업로드 + 변환 |
| 2 | `src/app/api/recordings/[id]/route.ts` | GET — 요약 결과 조회 |
...
| # | 파일 | 작업 |
|---|------|------|
| 1 | `src/app/api/reviews/route.ts` 확장 또는 별도 생성 route 검토 | POST — 복습 요약 생성 |
...
| # | 파일 | 작업 |
|---|------|------|
| 1 | `src/app/api/churn/route.ts` 또는 별도 cron route | POST — 일일 배치 계산 (Vercel Cron) |
| 2 | `src/app/api/churn/[id]/route.ts` | PATCH/POST — 이탈 처리 이벤트 |
...
| # | 파일 | 작업 |
|---|------|------|
| 1 | 민원 응답 API에 AI 초안 필드 추가 | `src/app/api/complaints/*` 확장 |
...
> 경로 네이밍은 새 `/api/ai/*` 계층을 추가하기보다, 현재 저장소의 canonical route를 우선 확장하는 방향으로 정리한다.
