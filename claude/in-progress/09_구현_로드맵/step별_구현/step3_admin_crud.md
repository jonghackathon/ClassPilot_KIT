...
### 3-5. 상담/이탈 관리 (Consultations + Churn)
...
| 4 | `src/app/api/churn/route.ts` | GET — 위험도/검색 필터 |
| 5 | `src/app/api/churn/[id]/route.ts` | PATCH — 위험도/점수 조정 |
| 6 | `src/hooks/useChurn.ts` | SWR wrapper |

**화면 연결:** `admin/churn` — 연락 기록, 이탈 처리, 위험 학생 우선순위
